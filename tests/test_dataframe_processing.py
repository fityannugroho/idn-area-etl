...
class TestIsTargetTable:
    ...
    def test_is_target_table_valid(self, sample_dataframe):
        """Test valid target table recognition."""
        assert is_target_table(sample_dataframe) is True

    def test_is_target_table_invalid(self, invalid_dataframe):
    ...
    def test_is_target_table_misparsed_headers(self):
        ...
        misparsed_df = pd.DataFrame([
            ["K o d e", "N a m a  P r o v i n s i"],
            ["11", "ACEH"]
        ])
...
class TestExtractAreaCodeAndNameFromTable:
    ...
    def test_extract_basic_data(self, sample_dataframe):
